...
def createSurgeries(reqs, waiting_slot):
    surgs = {}
    ...
    for i in range(len(reqs['A'])):
        ...
        tmp_surg.append(Surgery(0, reqs['A'][i], A_surg_time, waiting_slot))
    surgs['A'] = tmp_surg

    tmp_surg = []
    for i in range(len(reqs['B'])):
        B_surg_time = genSurgeryTime(B_surg_rate, 5)
        tmp_surg.append(Surgery(1, reqs['B'][i], B_surg_time, waiting_slot))
    surgs['B'] = tmp_surg

    tmp_surg = []
    for i in range(len(reqs['C'])):
        C_surg_time = genSurgeryTime(C_surg_rate, 10)
        tmp_surg.append(Surgery(2, reqs['C'][i], C_surg_time, waiting_slot))
    surgs['C'] = tmp_surg
